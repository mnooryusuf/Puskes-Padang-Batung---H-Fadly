<x-filament-panels::page>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        {{-- Card Kunjungan --}}
        <div class="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-4">
                <div class="p-3 bg-blue-100 dark:bg-blue-900 rounded-lg text-blue-600 dark:text-blue-300">
                    <x-heroicon-o-user-group class="w-8 h-8"/>
                </div>
                <div>
                    <h3 class="text-lg font-bold">Laporan Kunjungan (Detail)</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Daftar rinci pasien yang Anda tangani di Poli ini.</p>
                </div>
            </div>
            <div class="mt-6">
                {{ $this->cetak_kunjunganAction }}
            </div>
        </div>

        {{-- Card LB1 --}}
        <div class="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-4">
                <div class="p-3 bg-amber-100 dark:bg-amber-900 rounded-lg text-amber-600 dark:text-amber-300">
                    <x-heroicon-o-clipboard-document-check class="w-8 h-8"/>
                </div>
                <div>
                    <h3 class="text-lg font-bold">LB1 (10 Besar Penyakit)</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Statistik diagnosa penyakit terbanyak di Poli Anda.</p>
                </div>
            </div>
            <div class="mt-6">
                {{ $this->cetak_lb1Action }}
            </div>
        </div>

        {{-- Card Rekap Tindakan --}}
        <div class="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-4">
                <div class="p-3 bg-indigo-100 dark:bg-indigo-900 rounded-lg text-indigo-600 dark:text-indigo-300">
                    <x-heroicon-o-document-magnifying-glass class="w-8 h-8"/>
                </div>
                <div>
                    <h3 class="text-lg font-bold">Rekap Tindakan Medis</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Rekapitulasi tindakan dan pelayanan yang Anda berikan.</p>
                </div>
            </div>
            <div class="mt-6">
                {{ $this->cetak_rekap_tindakanAction }}
            </div>
        </div>

        {{-- Card Statistik Lab --}}
        <div class="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-4">
                <div class="p-3 bg-green-100 dark:bg-green-900 rounded-lg text-green-600 dark:text-green-300">
                    <x-heroicon-o-beaker class="w-8 h-8"/>
                </div>
                <div>
                    <h3 class="text-lg font-bold">Statistik Layanan Lab</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Statistik permintaan pemeriksaan penunjang (Laboratorium).</p>
                </div>
            </div>
            <div class="mt-6">
                {{ $this->cetak_statistik_labAction }}
            </div>
        </div>
    </div>

    <x-filament-actions::modals />
</x-filament-panels::page>
